...
namespace Remus
{
    ...
    public sealed class Command
    {
            ...
            object? obj = null)
        {
            ...
            {
                // for loops are faster than foreach, especially on arrays
                ...
                if (parameter.IsOptional)
                {
                    ...
                    _options.Add((optionalArgumentAttribute?.Name ?? parameter.Name!,
                        optionalArgumentAttribute?.Description ?? "N/A"));
                    continue;
                }
        ...
        private object?[] BindParameters(ICommandSender sender, InputMetadata inputData)
        {
            ...
            for (var i = 0; i < parameters.Length; ++i)
            {
                ...
                if (parameter.IsOptional)
                {
                    var optionName = parameter.GetCustomAttribute<OptionalArgumentAttribute>()?.Name ??
                ...
                else
                {
                    ...
                    {
                        if (parameter.ParameterType == typeof(bool) &&
                            inputData.Flags.Contains(flagAttribute.Identifier))
                        {
                            arguments[i] = true;
                        }
                    }
